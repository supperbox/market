  ...
  import { mapGetters } from 'vuex'
    ...
    computed: {
      ...
      isSelectAll: function () {
        // 如果所有的item中没有checked属性为false的，则查询结果为undefined，返回true，这个值将传入子组件checkButton，将其显示为选中状态
        return this.cartList.find(item => item.checked === false) === undefined;
      }
    },
    methods: {
      checkBtnClick: function () {
        // 1.判断是否有未选中的按钮，找到第一个checked属性为false的item
        let isSelectAll = this.$store.getters.cartList.find(item => !item.checked);
        // 2.有未选中的内容, 则全部选中
        if (isSelectAll) {
          // 对每一个item的checked属性赋值为true
          this.$store.state.cartList.forEach(item => {
            item.checked = true;
          });
        } else {
         // 对每一个item的checked属性赋值为false
          this.$store.state.cartList.forEach(item => {
            item.checked = false;
          });
        }
